...
class ShockGraphDataset(Dataset):
    ...
    def __init__(self,directory,dataset,norm_factors,n_shot,k_way,episodes=600,test_samples=15,node_app=False,edge_app=False,cache=True,symmetric=False,data_augment=False,flip_pp=False,grid=8):
        'Initialization'
        ...
        if dataset=='cifar100':
            ...
            print('Using mini imagenet train dataset')
    ...
    def __getitem__(self, anchor_index):
        ...
        few_shot_classes=np.random.choice(n_classes,self.k_way,replace=False)

        support_set=[]
        test_set=[]

        for label in few_shot_classes:
            sample_idx=self.label_to_indices[label]
            exemplar=np.random.choice(sample_idx,self.n_shot,replace=False)
            queries=np.random.choice(list(set(sample_idx)-set(exemplar)),self.test_samples,replace=False)
            
            support_set.extend(exemplar)
        ...
        for idx in support_set:
             features = self.sg_features[idx]
             label    = self.sg_labels[idx]
             spp_map  = self.grid_mapping[idx]
             graph    = self.sg_graphs[idx]
        ...
        return batched_graph,torch.tensor(label_values)
