...
class ShockGraphDataset(Dataset):
    ...
    def __getitem__(self, anchor_index):
        ...
        # get anchor data
        features_anchor = self.sg_features[anchor_index]
        label_anchor    = self.sg_labels[anchor_index]
        spp_map_anchor  = self.grid_mapping[anchor_index]
        ...
        negative_label = np.random.choice(list(set(self.class_mapping.values())-set([label_anchor])))
        negative_index = np.random.choice(self.label_to_indices[negative_label])

        # get positive data
        features_positive = self.sg_features[positive_index]
        label_positive    = self.sg_labels[positive_index]
        spp_map_positive  = self.grid_mapping[positive_index]

        # get negative data
        features_negative = self.sg_features[negative_index]
        label_negative    = self.sg_labels[negative_index]
        spp_map_negative  = self.grid_mapping[negative_index]

        
        # get anchor data
        graph_anchor            = self.sg_graphs[anchor_index]
        F_matrix_anchor         = np.copy(features_anchor[0])
        graph_anchor.ndata['h'] = torch.from_numpy(F_matrix_anchor)
        graph_anchor.ndata['x'] = torch.from_numpy(spp_map_anchor)
        
        # get positive data
        graph_positive            = self.sg_graphs[positive_index]
        F_matrix_positive         = np.copy(features_positive[0])
        graph_positive.ndata['h'] = torch.from_numpy(F_matrix_positive)
        graph_positive.ndata['x'] = torch.from_numpy(spp_map_positive)
        
        # get negative data
        graph_negative            = self.sg_graphs[negative_index]
        F_matrix_negative         = np.copy(features_negative[0])
        graph_negative.ndata['h'] = torch.from_numpy(F_matrix_negative)
        graph_negative.ndata['x']   = torch.from_numpy(spp_map_negative)

        return graph_anchor,graph_positive,graph_negative
...
def collate(samples,device_name):
    ...
    batched_graph = dgl.batch(anchors)
