...
from .ef_conv import EfConv

class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes,hidden_layers,edge_dim,aggregate,readout,
        ...
        self.layers.append(EfConv(in_dim,hidden_dim,edge_dim,aggregate,bias=True,activation=activation_func))
        self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
                
        # hidden layers
        for k in range(0,hidden_layers):
            self.layers.append(EfConv(hidden_dim,hidden_dim,edge_dim,aggregate,bias=True,activation=activation_func))
            self.batch_norms.append(nn.BatchNorm1d(hidden_dim))
