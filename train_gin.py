...
from models.gin_sg_model import Classifier
...
def main(args):
    ...
    prefix='gin_sg_model_'+dataset+'_'+bdir+'_'+str(args.n_layers)+'_'+str(args.n_hidden)+'_'+args.neighbor_pooling_type+'_'+args.graph_pooling_type
    ...
    model = Classifier(args.n_layers,
                       args.num_mlp_layers,
                       num_feats,
                       args.n_hidden,
                       n_classes,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GraphGIN')
    ...
    parser.add_argument('--graph_pooling_type', type=str,default="sum",
                        choices=["sum", "mean", "max"],help='type of graph pooling: sum, mean or max')
    parser.add_argument('--neighbor_pooling_type', type=str,default="sum",
                        choices=["sum", "mean", "max"],help='type of neighboring pooling: sum, mean or max')
